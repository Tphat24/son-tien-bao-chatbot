...
const schema = z.object({
  ...
  AI_MODEL_ATTEMPT_TIMEOUT_MS: z.coerce.number().int().min(3000).max(30000).default(12000),
  AI_FAILOVER_BUDGET_MS: z.coerce.number().int().min(5000).max(45000).default(32000),
  AI_RETRY_BASE_DELAY_MS: z.coerce.number().int().min(0).max(5000).default(400),
  AI_MAX_PRODUCTS: z.coerce.number().int().min(1).max(5).default(5),
  AI_MAX_KNOWLEDGE_DOCS: z.coerce.number().int().min(1).max(8).default(5),
  RAG_ENABLED: boolString('true'),
  RAG_TENANT_ID: z.string().trim().min(1).max(100).default('default'),
  RAG_EMBEDDING_MODEL: z.string().trim().min(1).default('gemini-embedding-001'),
  RAG_TOP_K: z.coerce.number().int().min(1).max(20).default(8),
  RAG_MATCH_THRESHOLD: z.coerce.number().min(0).max(1).default(0.62),
  RAG_CHUNK_SIZE: z.coerce.number().int().min(500).max(8000).default(1800),
  RAG_CHUNK_OVERLAP: z.coerce.number().int().min(0).max(1000).default(240),
